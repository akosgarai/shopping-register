...
class SparParserService extends AbstractParserService
{
    ...
    private function parseSparReceipt() {
        ...
        $this->setupTaxNumber();
        $firstLineAfterItems = $this->parseSparItemLines($this->lines, $this->readLineIndex);
    ...
    private function parseSparItemLines($lines, $from)
    {
        ...
        for ($i = $from; $i < $numberOfLines; $i++) {
            if (!preg_match('/^[A-Z0-9]{1,3} /', $lines[$i])) {
                ...
                // If the line contains the current total, then continue. The current total starts with 'RESZOSSZESEN'.
                if (strpos($lines[$i], 'RESZOSSZESEN') !== false) {
                    continue;
                }
                if ($itemParsing) {
                    return $i;
            ...
            if (strpos(strtolower($lines[$i+1]), 'kg') !== false) {
                $rawFloatNumber = str_replace(',', '.', trim(substr($lines[$i+1], 0, strpos($lines[$i+1], ' '))));
